...
class SessionsService:
    ...
    async def abandon_session(self, session_id: uuid.UUID) -> SessionResponse:
        session = await self.repo.get(session_id)
        if session is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Sesión no encontrada.")
        if session.estado in ("cerrada", "abandonada", "anulada"):
            return serialize_session(session)
        saved = await self.repo.abandon(session)
        return serialize_session(saved)
